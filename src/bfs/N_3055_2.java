...
public class N_3055_2 {
    ...
    void increaseWater() //원래 맵에다가 물을 한번 증가시키는것 
    {
       int water[][] = new int[51][51];
       for (int i = 1; i <= X; i++) {
          for (int j = 1; j <= Y; j++) {
             water[i][j] = map[i][j];
          }
    ...
    int func(int start_x, int start_y) // 큐 써서 이동하는 함수 
    {
        ...
        while (!x_queue.isEmpty())
        {
            ...
            if(map[x][y] == water_cnt){
            	System.out.println("값:"+ map[x][y]);
            	System.out.println("값:"+water_cnt);
               increaseWater();
               water_cnt++;
